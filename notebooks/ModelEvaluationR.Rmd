...
```{r}
baseball_lm <- lm(Score ~ . - teamAbbr - League - RBI - OPS - X,data=baseball_df)
...
```{r}
augment(baseball_lm,data=baseball_df) %>% ggplot(aes(x=.fitted,y=.resid)) + geom_point()
```


```{r}
augment(baseball_lm,data=baseball_df) %>% 
...
baseball_fit_train <- lm(Score ~ . - teamAbbr - League - RBI - OPS - X,data=baseball_train)
...
baseball_train_lm <- train(Score ~ . - teamAbbr - League - RBI - OPS - X,data=baseball_train,method="lm",trControl=tC)
