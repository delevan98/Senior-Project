...
sns.set(rc={'figure.figsize':(5.5,3.9)})
...
We tested the linear regression model using the unseen processed test data. In this case, the actual test error was calculated to be **1.049** which was slightly higher than the training error and the cross validation estimated error. Again the model performs extremely well on all data is is, on average, only 1 point off of the actual score for a game. When the model was applied to Opening Day 2019 games, it achieved a **3.5** RMSE value which is a sharp drop from our previous results. Again this can be attributed to using 10-game averages to predict future games because we do not have data for future games yet. There is also a high variability in score due to the many factors at play in a baseball game that vary from team to team and game to game. Our model may not be able to perfectly predict the score because there just is not enough variables to encapsulate all of the factors attributed to score. However, when the scores were used to predict the winner of the game, a classification task, we achieved a **61%** accuracy score. Although this is slightly less than the accuracy achieved by the logistic regression model, it is still an interesting result that may need more research.
